...
export function RolesPage({ principal }: { principal: Principal }) {
  ...
  return <div className="space-y-6"><div><h1 className="text-2xl font-semibold">Roles</h1><p className="text-sm text-muted-foreground">Roles reconocidos por la API. Se asignan en Microsoft Entra y se sincronizan en el siguiente login.</p></div>{list.isError && <ErrorMessage error={list.error} />}<Table><TableHeader><TableRow><TableHead>Código</TableHead><TableHead>Nombre</TableHead></TableRow></TableHeader><TableBody>{list.isPending && <TableRow><TableCell colSpan={2}>Cargando roles…</TableCell></TableRow>}{list.isSuccess && !list.data.length && <TableRow><TableCell colSpan={2}>No hay roles para mostrar.</TableCell></TableRow>}{list.data?.map((role) => <TableRow key={role.code}><TableCell>{role.code}</TableCell><TableCell>{role.name}</TableCell></TableRow>)}</TableBody></Table></div>
}
