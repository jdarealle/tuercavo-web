(() => async (page) => {
  ...
  check(await page.getByRole('link', { name: 'Dashboard' }).count() === 1, 'Sidebar: falta Dashboard')
  await page.locator('[data-slot="card"]').filter({ has: page.getByRole('heading', { name: 'Categorías' }) }).getByText('1').waitFor()
